# R script for taking the parcels output files and merges them into R files
# Run on the UNSW Katana HPC
...
#file_names <- list.files("../../srv/scratch/z3374139/", pattern=".nc")
#file_names2 <- paste("../../srv/scratch/z3374139/", file_names, sep="")
#file_names2
#save_names <- file_path_sans_ext(file_names)
#save_names

#list_full_data_endpoints <- list()
#list_full_data <- list()
#list_shelf_data_endpoints <- list()
# Start the clock!
#ptm <- proc.time()
...
file_names <- list.files("../../srv/scratch/z9902002/out/", pattern=".nc")
file_names2 <- paste("../../srv/scratch/z9902002/out/", file_names, sep="")
#file_names2

save_names <- file_path_sans_ext(file_names)
save_names2 <- paste0("../../srv/scratch/z3374139/Forward/", save_names, sep="")

file_names2[ID]
#for(i in 1:length(file_names2)){
  mydata <- nc_open(file_names2[ID])
  #mydata <- nc_open("katana output/2019 Run/Daily/Tailor_Parcels_Output_Year1994_For.nc")
  ...
  total_data <- total_data[with(total_data, order(Particle, Time)), ]
  ...
  for (i in 1:nrow(total_data)) {
    if (total_data$DayofLife[i] == 0) {
      ...
      total_data$Release_Year[i] <- total_data$Year[i]
      total_data$Cohort[i] <- paste(total_data$Latitude[i], total_data$Year[i], total_data$time[i]) 
    }
  }
  ...
  total_data$Release_Year <- na.locf(total_data$Release_Year, na.rm=FALSE) # replace NA with last non-NA observation carried forward
...
#write.csv(total_data, filename = paste(save_names2[ID], ".csv", sep=""), row.names = F)
